    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $newcoupon=array(
                ...
                'description' => 'Mã giảm giá 50.000 đ tự động khi đăng ký thành công',
                'created' => $today,
                'orders' => 0,
                'trash' => 1,
                'status' => 1,
            ...
            $config['smtp_user']    = 'ngocthai31082003@gmail.com';
            $config['smtp_pass']    = 'isqn ofie cuxh hnwf';
            ...
            $this->email->from('ngocthai31082003@gmail.com', 'NgocThai');
            $this->email->to($email);
            $this->email->subject('NgocThai - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng , Cửa hàng tặng bạn 1 mã giảm giá giảm 50.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']=' Cập nhật mật khẩu mới';
